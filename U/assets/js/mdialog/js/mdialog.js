...
function malert3(title, content) {
    var tmp = '<div class="ui-dialog show"><div class="ui-dialog-cnt" style="width:200px;"><div class="ui-dialog-bd"><div><h4>' + title + '</h4><div>' + content + '</div></div></div><div class="ui-dialog-ft ui-btn-group"><button type="button" class="select" style="color:#fb7166;font-size: 18px;height:60px;line-height:60px;border:0;" onclick="closemsgs();">提现成功</button></div></div></div>';
    $("body").append(tmp);
}
function malert4(title, content) {
    var tmp = '<div class="ui-dialog show"><div class="ui-dialog-cnt" style="width:200px;"><div class="ui-dialog-bd"><div><h4>' + title + '</h4><div>' + content + '</div></div></div><div class="ui-dialog-ft ui-btn-group"><button type="button" class="select" style="color:#fb7166;font-size: 18px;height:60px;line-height:60px;border:0;" onclick="closemsgs();">提现失败</button></div></div></div>';
    $("body").append(tmp);
}
//弹出框(内容)
